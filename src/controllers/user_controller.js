  ...
  try {
    ...
    const mail = {
      ...
      html: `
                ...
                <a href='http://localhost:3000/verification?token=${userToken}'>Verify your account</a>
            `,
    };
    ...
    if (USER.length) {
      ...
      return res
        .header("authorization", `Bearer ${userToken}`, "email", `${email}`)
        .send(responseStatus);
    }
  } catch (error) {
...
module.exports.getUser = async (req, res) => {
  try {
    ...
    const userToken = userTokenHeader.split(" ")[1];
    const emailToken = jwt.verify(userToken, process.env.SECRET_KEY).email;
...
module.exports.login = async (req, res) => {
  ...
  try {
    ...
    res.header("authorization", `Bearer ${userToken}`).send(responseStatus);
  } catch (error) {
    console.log("error: ", error);
  }
};
...
module.exports.keepLogin = async (req, resp) => {
  ...
  try {
    ...
    if (!userData.length) {
      throw new createError(httpStatus.NOT_FOUND, `admin with email from token doesn't found`)
    }

    //send respond to client
    delete userData[0].password
    const respond = new createRespond(httpStatus.OK, httpStatus.OK, 'Keep User Login', 1, 1, userData[0])
    resp.status(respond.status).send(respond.data)

  } catch (err) {
    const throwError = err instanceof createError
    if (!throwError) {
      new createError(httpStatus.INTERNAL_SERVICE_ERROR, httpStatus.INTERNAL_SERVICE_ERROR)
    }
...
module.exports.resetPassword = async (req, resp) => {
  ...
  try {
    ...
    if (error) {
      ...
      resp.status(responseStatus.status).send(responseStatus);
      throw new createError(httpStatus.BAD_REQUEST, error.details[0].message);
    }
    ...
    await database.execute(updatePassword, [hashPassword, email]);
